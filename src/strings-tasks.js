...
function removeLastOccurrences(str, value) {
  ...
  return str.slice(0, position) + str.slice(value.length + position);
}
...
function sumOfCodes(str) {
  ...
  return str.split('').reduce((acc, elem) => acc + elem.charCodeAt(0), 0);
}
...
function formatTime(minutes, seconds) {
  ...
  const formattedMinutes =
    minutes < 10 ? strMinutes.padStart(2, '0') : strMinutes;
  const formattedSeconds =
    seconds < 10 ? strSeconds.padStart(2, '0') : strSeconds;
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    ...
    let newStr;
    if (code >= 65 && code <= 90) {
      newStr = String.fromCharCode(((code - 65 + 13) % 26) + 65);
    }
    if (code >= 97 && code <= 122) {
      newStr = String.fromCharCode(((code - 97 + 13) % 26) + 97);
    }
    return newStr;
  });
}
